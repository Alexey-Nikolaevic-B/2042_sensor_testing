...
### ⚠️ Важно: формат boolean в пользовательских SDF

Если вы используете **собственный SDF-файл** (не из папки `sensors/`), проверьте значения тега `<always_on>` и всех других boolean-параметров внутри `<sensor>` блоков. Они должны быть записаны как `true` / `false`, а **не** как `1` / `0`:
...
**Почему это важно:** парсер SDF в Gazebo (библиотека `sdformat`) в своей строгой проверке boolean-значений принимает **только** строки `true` / `false`. Числовые `1` / `0` могут работать на одной машине и ломаться на другой — всё зависит от версии `sdformat`. Если парсер отвергнет значение, в логах появится:

```
[ERRO] simulator: Error [Param.cc:329] Invalid boolean value
[ERRO] simulator: Error Code 8 Msg: Error reading element <always_on>
```

При этой ошибке **весь блок `<sensor>` не создаётся**: плагин не загружается, топики не публикуются, тесты падают без внятной причины. Поэтому для всех boolean-параметров в SDF (`<always_on>`, `<static>`, `<gravity>`, `<visualize>` и т.д.) используйте только `true` / `false`.
...
### Приложение крашится / зависает после нескольких тестов (`Too many open files`)

Если в логах появляется `OSError: [Errno 24] Too many open files` или `GLib-ERROR: Creating pipes for GWakeup` — значит исчерпан системный лимит открытых файловых дескрипторов. Каждый тест открывает ROS-сокеты, pipes Gazebo и другие файлы. При стандартном лимите 1024 на несколько тяжёлых тестов (C9, C10, C11) может не хватать.

Решение — поднять лимит перед запуском:

```bash
# 1. Проверить текущий лимит
ulimit -n

# 2. Если меньше 4096 — поднять
ulimit -n 4096

# 3. Запустить приложение (в том же терминале!)
source /opt/ros/noetic/setup.bash
source catkin_ws/devel/setup.bash
source venv/bin/activate
python3 __main__.py
...
**Важно:** `ulimit` действует только в текущем терминале. В новом терминале лимит снова 1024.

Чтобы изменение было постоянным для вашего пользователя, добавьте в конец `~/.bashrc`:
