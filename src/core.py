...
class Core:
    ...
    # ── new: clean shutdown without sys.exit ──────────────────────────

    def shutdown(self) -> None:
        """Terminate simulator processes without calling sys.exit."""
        try:
    ...
    # ── new: unified test execution (replaces test_runner.py) ─────────
    ...
    def run_sensor_tests(
        ...
        """Run test(s) for a single sensor. Returns (overall, report_path, payload)."""
        report_dir = self._report_dir(sensor_name)
        sensor = make_sensor(sensor_type, sensor_name, CONFIG)
        if sensor is None:
            payload = self._fail_payload(sensor_type, sensor_name,
        ...
        if run_suite or not test_name:
            ...
            tests = {n: fn for n, fn in discovered.items() if n == test_name}
            selected_mode = "single"

        if not tests:
            payload = self._fail_payload(sensor_type, sensor_name, "single",
        ...
        for tname, tfn in tests.items():
            try:
                result = tfn(self.simulator)
                test_results.append({"name": tname, "status": "PASS", "result": result})
            except Exception as exc:
                entry: Dict[str, Any] = {"name": tname, "status": "FAIL", "error": str(exc)}
                details = self._collect_test_diagnostics(sensor, report_dir, tname)
                if details:
    ...
    def run_sensor_suite(
        ...
        """High-level entrypoint (replaces test_runner.main). Returns exit code."""
        if all_sensors:
        ...
        for sname in sensor_names:
            overall, report_path, payload = self.run_sensor_tests(
                sensor_type, sname, test_name=test_name, suite=suite,
            )
    ...
    def _collect_test_diagnostics(
        self, sensor: Sensor, report_dir: str, test_name: str,
    ) -> Dict[str, Any]:
        ...
        if callable(getter):
            try:
                expected_topics = [str(t) for t in getter() if str(t).strip()]
            except Exception:
                expected_topics = []

        scene_diag_getter = getattr(sim, "get_last_scene_diagnostics", None)
        if callable(scene_diag_getter):
            try:
                diag = scene_diag_getter()
                if diag:
                    details["scene"] = diag
            except Exception:
                pass

        runtime_getter = getattr(sim, "collect_failure_diagnostics", None)
        if callable(runtime_getter):
            try:
                rt = runtime_getter(expected_topics=expected_topics)
                if rt:
                    details["runtime"] = rt
            except Exception:
                pass
        ...
        if expected_topics:
            capture_fn = getattr(sim, "capture_debug_frame", None)
