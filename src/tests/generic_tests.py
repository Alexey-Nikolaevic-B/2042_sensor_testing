...
def camera_check(simulator, sensor, progress_cb=None) -> dict:
    ...
    logger.info("Capturing image from %s ...", primary_topic)
    msgs = sensor.capture_data(Image, topic=primary_topic,
                               window=5.0, timeout=1.0, warmup=0.0,
                               simulator=simulator)
    if not msgs:
        result["error"] = f"No image received on {primary_topic}"
        result["duration"] = round(time.time() - t0, 2)
        logger.error("No image received on %s", primary_topic)
        return result
    msg = msgs[-1]
    logger.info("Image captured: %dx%d, encoding=%s", msg.width, msg.height, msg.encoding)
