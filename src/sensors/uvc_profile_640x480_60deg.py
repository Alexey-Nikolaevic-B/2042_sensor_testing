...
class UvcProfile640x48060Deg(MonoCamera):
    ...
    def __init__(self, CONFIG):
        ...
        worlds_dir = os.path.join(root, "catkin_ws", "src", "scenario_test_pkg", "worlds")
        self.test_to_world = {
    ...
    def _report_path(self) -> str:
        return os.path.join(self._results_dir(), "report.json")
    ...
    @staticmethod
    def _test_result(test_id: str, status: str, metrics: Dict[str, Any], artifacts: List[str], error: str | None = None) -> Dict[str, Any]:
        result: Dict[str, Any] = {
            "id": test_id,
            "status": status,
            "metrics": metrics,
    ...
    def c1_size_order_test(self, simulator) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._test_result("C1", status, metrics, artifacts)

        except Exception as exc:
            return self._test_result("C1", "FAIL", metrics, artifacts, error=str(exc))
    ...
    def c4_geometries_presence_test(self, simulator) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._test_result("C4", status, metrics, artifacts)

        except Exception as exc:
            return self._test_result("C4", "FAIL", metrics, artifacts, error=str(exc))
    ...
    def c7_occlusion_test(self, simulator) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._test_result("C7", status, metrics, artifacts)

        except Exception as exc:
            return self._test_result("C7", "FAIL", metrics, artifacts, error=str(exc))

    def camera_suite(self, simulator) -> Dict[str, Any]:
        """Convenience метод: выполняет все профильные `*_test` и пишет общий report."""
        tests = [
            self.c1_size_order_test(simulator),
            self.c4_geometries_presence_test(simulator),
            self.c7_occlusion_test(simulator),
        ]
        overall = "PASS" if all(t.get("status") == "PASS" for t in tests) else "FAIL"

        payload = {
            "camera": self.sensor_name,
